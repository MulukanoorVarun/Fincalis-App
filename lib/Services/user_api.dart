...
class UserApi {
  // static const host = "http://192.168.0.169:8060";
  static const host = "https://app.fincalis.in";
  ...
  static Future<RecieveOtp?> ReceiveOtp(String mobile) async {
    try {
      ...
      final url = '${host}/otp/send?mobile=$mobile';
      final response = await http.post(Uri.parse(url), headers: headers);

      if (response.statusCode == 200) {
        print("ReceiveOtp response: ${response.body}");
        return RecieveOtp.fromJson(jsonDecode(response.body));
      } else {
  ...
  static Future<GetCreditScoreReportModel?> GetCreditScoreOTPVerificationApi(String otp,String transid,String userid) async {
   ...
   static Future<RegisterationModel?>RegistrationApi(String name,String email,String Userid) async {
     try {
       ...
       if (res != null) {
         ...
         return RegisterationModel.fromJson(jsonDecode(res.body));
       } else {
         print("Null Response");
         return null;
       }
  ...
  static Future<PincodeResponse?> fetchPincodeDetails(pincode) async {
    final response = await http.get(Uri.parse('https://api.postalpincode.in/pincode/${pincode}'));
    if (response.statusCode == 200) {
      return PincodeResponse.fromJson(jsonDecode(response.body)[0]);
    } else {
      throw Exception('Failed to load pincode details');
    }
  }
