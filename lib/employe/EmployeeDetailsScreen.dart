...
import '../model/PincodeResponse.dart';
...
class _EmploymentDetailsState extends State<EmploymentDetails> {
  ...
  void _validateFields() {
    ...
    if (_officialEmailController.text.isEmpty ||
        !RegExp(r'\S+@\S+\.\S+').hasMatch(_officialEmailController.text)) {
    ...
    if (!declare) {
      validateDeclaration =
      "Please check the box to confirm that you have agreed to the declaration.";
    }
  ...
  Future<void> GetAddressDetails() async {
    await UserApi.fetchPincodeDetails(_pinCodeController.text).then((data) => {
      if (data != null)
        {
          setState(() {
            if (data.status =="Success") {
              postOffices=data.postOffices??[];
              print("postOffices:${postOffices[0].name},${postOffices[0].district},${postOffices[0].state}");
              for(int i=0;i<postOffices.length;i++){
                _companyAddressController.text="${postOffices[0].name}, ${postOffices[0].district}, ${postOffices[0].state}";
              }
  ...
  Future<void> SubmittingEmployeeDetails(status) async {
    var Userid = await PreferenceService().getString('user_id');
    await UserApi.SubmittingEmployeeDetailsApi(
            ...
            if (data.settings?.success == 1) {
              ...
              if (result != null) {
                ...
                setState(() {
                  optionsMap1 = map;
                  if (map.isNotEmpty) {
                    selectedValue1 = map.keys
                        .first; // Set the initial value to the first key
                    selectedKey1 = map.values
                        .first; // Set the initial key to the first value
                  }
                });
              } else {
                ...
                DropdownButtonFormField<String>(
                  ...
                  onChanged: (value) {
                    setState(() {
                      ...
                      selectedKey1 = optionsMap1[
                      value!]; // Update selectedKey based on selectedValue
                      industry_type = selectedValue1!;
                      _validateIndustryType = "";
                    });
                    // You can now use selectedKey to send to the server
                    print(
                        'Selected key to send to server: $selectedValue1');
                  },
                  items: optionsMap1.entries.map((entry) {
                    return DropdownMenuItem<String>(
                      value:
                      entry.key, // The key is used as the value
                      child: Text(
                        entry
                            .value, // The value is displayed as text
                        style: TextStyle(
                          fontSize: 16,
                          letterSpacing: 0,
                          height: 1.2,
  ...
  Widget _buildStep(int stepIndex) {
    bool isCompleted = (isworkcompleted) && stepIndex < 2 ||
        (widget.isBasicCompleted) && stepIndex < 1;
    Color stepColor =
  ...
  Widget _buildSeparator(int separatorIndex) {
    bool isCompleted = (isworkcompleted) && separatorIndex < 2 ||
        (widget.isBasicCompleted) && separatorIndex < 1;
